...
/* GET home page. */
router.post('/dencrypt', function(req, res, next) {
    var result = req.body
    res.send(dencrypt(result.data,result.hkey));
});
router.get('/articleid/:id', function(req, res, next) {
    var id = req.params.id.replace(':','')
    console.log(id,typeof req.params)

    res.send(get_hkey_value(id));
});
